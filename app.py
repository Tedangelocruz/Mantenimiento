...
# Optional: exclude some fichas from the list (case-insensitive)
...
def get_gs_client():
    ...
    # Local dev fallback: service_account.json file in app folder
    ...
    st.error("No se encontraron credenciales. Agrega GSERVICE_ACCOUNT_JSON en Secrets o 'service_account.json' local.")
    st.stop()
...
def cache_decorator():
    if hasattr(st, "cache_data"):
        # TTL corto para que todos vean cambios rápidamente
...
def load_df_from_sheet(sheet_id: str) -> pd.DataFrame:
    ...
    # Uses first worksheet (gid=0). Change to .worksheet("TabName") if needed.
    ...
    for col in required:
        if col not in df.columns:
            st.error(f"Falta columna '{col}'. Columnas encontradas: {list(df.columns)}")
            st.stop()
...
def save_df_to_sheet(sheet_id: str, df: pd.DataFrame):
    ...
    ws = client.open_by_key(sheet_id).sheet1
    df2 = df.fillna("")
    # Overwrite entire sheet with current columns/values
    ws.clear()
...
def update_sheet_date(sheet_id: str, ficha: str, new_date: date) -> bool:
    ...
    # Only write the original columns back (avoid computed columns)
...
def compute_status(df: pd.DataFrame, threshold_days: int) -> pd.DataFrame:
    ...
    out["Estado"] = out["Días desde último mant."].apply(lambda d: "Verde" if (d is not None and int(d) < threshold_days) else "Rojo")
    return out
...
def list_view(sheet_id: str):
    ...
    st.dataframe(ft.sort_values(by=["Estado","Próximo Mantenimiento","Ficha"]), use_container_width=True)
...
def detail_view(sheet_id: str, ficha: str):
    ...
    if ok:
        # Save lightweight metadata (local json) - optional
        ...
        # Update sheet last-maintenance date
