...
def compute_status(df: pd.DataFrame, threshold_days: int) -> pd.DataFrame:
    today = date.today()
    out = df.copy()
    # days since last maintenance; robust to NaT
    diffs = []
    for val in out["Fecha_parsed"]:
        try:
            ...
            diffs.append(None)
    out["Días desde último mant."] = diffs

    # Normalize to numeric safely
    out["Días desde último mant."] = pd.to_numeric(out["Días desde último mant."], errors="coerce")
